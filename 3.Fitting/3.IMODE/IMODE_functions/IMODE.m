...
function [x, xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq,current_eval,res_det,F,cr ] = ...
    IMODE( f,x,xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq, xmin, xmax,  n,...
    PopSize,  current_eval,res_det,Printing,Max_FES, G_Max, gg,F,cr)
...
%% evaluate
fitx_new = zeros(1,size(ui,1));
for i = 1 : size(ui,1)
        fitx_new(i) = f(ui(i,:));
end
% [fitx_new,index]=sort(fitx_new);
% ui = ui(index,:);
